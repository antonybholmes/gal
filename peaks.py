...
def duplicate_peaks(type, file):
    ...
    overlap_type_column = text.find_index(header, "Relative To Gene")
...
class NClosestGenes(genomic.Annotation):
    ...
    def __init__(
        ...
        self._refseq_start: tss.RefSeqTss = refseq_start
        self._n: int = n
        ...
        for i in range(start, self._start + self._n):
            ...
            self._header.append(f"#{i} Closest {headings.GENE_SYMBOL}")
    ...
    def update_row(self, location: genomic.Location, row_map: Mapping[str, Any]):
        ...
        tss_genes = self._refseq_start.get_n_closest_genes(location, self._n)
        # )  # n=100)  #
        ...
        for tss_gene in tss_genes:
            ...
            if ci >= self._start:
                ...
                if isinstance(row_map, dict):
                    row_map[
                        f"#{ci} Closest {headings.REFSEQ_ID}"
                    ] = gene_annotation.refseq
                    row_map[
                        f"#{ci} Closest {headings.ENTREZ_ID}"
                    ] = gene_annotation.entrez
                    row_map[
                        f"#{ci} {headings.CLOSEST_GENE_SYMBOL}"
...
class AnnotateGene(genomic.Annotation):
    ...
    def update_row(self, location: genomic.Location, row_map: Mapping[str, Any]):
        ...
        for gene in genes:
            gene_annotation = self._refseq_genes.get_gene(gene.variant_id)
...
class AnnotatePeak:
    ...
    def parse(self, file: str) -> pd.DataFrame:
        # reset header for each file although cache it
        self._header = []
        ...
        if "TF_targets" in file:
        ...
        elif "seacr" in file and "_vs_" not in file:
            # seacr file must be seacr output and not some hybrid
            # file that happens to contain the word seacr
            ...
            # check if first col appears to be a locations
            tokens = f.readline().strip().split("\t")
            ...
            has_width_col = (
                len(list(filter(lambda x: "width" in x.lower(), file_header))) > 0
            )
        ...
        for line in f:
            ...
            if first_col_is_loc:
                ...
                location = genomic.Location(tokens[0], int(tokens[1]), int(tokens[2]))

            # chr = tokens[0]

            # Skip chrM
            if (
                "chrM" in location.chr
                or "chrGL" in location.chr
                or "chrK" in location.chr
                or "_" in location.chr
            ):
                continue
            ...
            if "TF_targets" in file:
                ...
                q = float(tokens[8])
                # max_score = float(tokens[4])
                annotation = [str(location), width, p, q, score]
                row_map[WIDTH_HEADING] = width
                ...
                p1 = tokens[5]
                p2 = tokens[6]
                annotation = [str(location), width, score, max_score, overlaps, p1, p2]
                row_map[WIDTH_HEADING] = width
                row_map["Total Score"] = score
                row_map["Max Score"] = max_score
                ...
                # first 2 as not part of real file header
                # for i in range(2, len(file_header)):
