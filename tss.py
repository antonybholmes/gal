...
class RefSeqTss(genomic.Annotation):
    ...
    def __init__(self, name: str, file: str, refseq_genes: genes.RefSeqGenes, prom_ext_5p: int, prom_ext_3p: int, bin_size: Optional[int] = 1000):
        super().__init__(name)
        ...
        self._refseq = RefSeqTssClassificationFactory.getInstance(
            file, prom_ext_5p, prom_ext_3p)
        self._promoter_type = genomic.promoter_type(prom_ext_5p, prom_ext_3p) #f'(prom=-{prom_ext_5p / 1000}/+{prom_ext_3p / 1000}kb)'
        self._header = []
