...
class TssGene:
    def __init__(self, variant_id: str, types: set[str], d: int):
    ...
    def __str__(self):
        return self._variant_id + ":" + ",".join(self._types) + ":" + str(self._d)


class TssGenes:
    def __init__(self, d: int, genes: list[TssGene]):
        self._d = d
        self._genes = genes
...
class RefSeqTssClassification:
    ...
    def __init__(self, file, prom_ext_5p, prom_ext_3p):
        ...
        print(f"RefSeqTssClassification: Loadings from {file}...", file=sys.stderr)
        ...
        for line in f:
            ...
            variant_id = genes.create_variant_id(refseq, chr, start, end)
    ...
    def classify(self, variant_id, peak_mid_point):
        ...
        for var_id in self._variants[refseq]:
            ...
            if strand == "+":
                d = peak_mid_point - gene_start
            else:
                d = gene_end - peak_mid_point
            ...
            if strand == "+":
                if peak_mid_point >= promoter_start and peak_mid_point <= gene_end:
                    within_bounds = True
            else:
                # negative strand
                if peak_mid_point >= gene_start and peak_mid_point <= promoter_end:
                    within_bounds = True

            if within_bounds:
                # Peak must be within gene boundary to be called promoter, exon or intron
                if peak_mid_point >= promoter_start and peak_mid_point <= promoter_end:
                ...
                for i in range(0, len(self._exon_starts[var_id])):
                    if (
                        peak_mid_point >= self._exon_starts[var_id][i]
                        and peak_mid_point <= self._exon_ends[var_id][i]
                    ):
                        types.add("exonic")
                        in_exon = True
                        break
                ...
                if (
                    not in_exon
                    and peak_mid_point >= gene_start
                    and peak_mid_point <= gene_end
                ):
                    types.add("intronic")
...
class RefSeqTssClassificationFactory:
    _classifiers = collections.defaultdict(lambda: collections.defaultdict(lambda: {}))

    @staticmethod
    def getInstance(file: str, prom_ext_5p: int, prom_ext_3p: int):
        if (
            prom_ext_3p
            not in RefSeqTssClassificationFactory._classifiers[file][prom_ext_5p]
        ):
            RefSeqTssClassificationFactory._classifiers[file][prom_ext_5p][
                prom_ext_3p
            ] = RefSeqTssClassification(file, prom_ext_5p, prom_ext_3p)

        return RefSeqTssClassificationFactory._classifiers[file][prom_ext_5p][
            prom_ext_3p
        ]
...
class RefSeqAnnotation:
    ...
    def __init__(
        self,
        file: str,
        prom_ext_5p: int = 2000,
        prom_ext_3p: int = 1000,
        bin_size: int = 1000,
    ):
        self._metadata = {}
        ...
        self._metadata["version"] = re.search(r"v([A-Za-z0-9]+)", file).group(1)
        ...
        for line in f:
            ...
            transcript_id = genes.create_variant_id(refseq, chr, start, end)
    ...
    def annotate_location(self, location) -> list[TssGene]:
...
class RefSeqAnnotationFactory:
    ...
    def getInstance(
        ...
        if (
            bin_size
            not in RefSeqAnnotationFactory._classifiers[file][prom_ext_5p][prom_ext_3p]
        ):
            RefSeqAnnotationFactory._classifiers[file][prom_ext_5p][prom_ext_3p][
                bin_size
            ] = RefSeqAnnotation(file, prom_ext_5p, prom_ext_3p, bin_size)

        return RefSeqAnnotationFactory._classifiers[file][prom_ext_5p][prom_ext_3p][
            bin_size
        ]
...
class RefSeqTss(genomic.Annotation):
    ...
    def __init__(
        ...
        self._gene_to_start_index_map = collections.defaultdict(
    ...
    def update_row(self, location: genomic.Location, row_map: Mapping[str, Any]):
        ...
        gene_annotation = self._refseq_genes.get_gene(tss_gene.variant_id)

        ret = [
            gene_annotation.refseq,
            gene_annotation.entrez,
            gene_annotation.symbol,
            ",".join(reversed(sorted(tss_gene.types))),
            str(tss_gene.d),
        ]
    ...
    def get_closest_gene(self, location: genomic.Location) -> TssGene:
        ...
        if mid_point < self._starts[location.chr][ps]:
            return self.get_annotation(
                location.chr, mid_point, self._starts[location.chr][ps]
            )

        if mid_point > self._starts[location.chr][pe]:
            return self.get_annotation(
                location.chr, mid_point, self._starts[location.chr][pe]
            )
    ...
    def get_n_closest_genes(
        self,
        location: genomic.Location,
        n: int = 5,
        are_unique_symbols: bool = True,
    ) -> list[TssGene]:
        """
        ...
        idx = self._gene_to_start_index_map[location.chr][closest_gene.variant_id]

        # start of closest gene
        ref_start = self._starts[location.chr][idx]
        d = abs(ref_start - mid_point)
        gene_annotation = self._refseq_genes.get_gene(closest_gene.variant_id)
        symbol = gene_annotation.symbol
        used = {symbol}
        variant_ids = collections.defaultdict(list)
        variant_ids[d].append(closest_gene.variant_id)

        found = 0

        if len(self._starts[location.chr]) > 1:
            idx1 = idx  # max(0, idx - 1)
            idx2 = idx  # min(self._starts[location.chr].size - 1, idx + 1)
            max_index = len(self._starts[location.chr]) - 1

            while found < n:
                tidx1 = idx1 - 1  # max(0, idx1 - 1)
                tidx2 = idx2 + 1  # min(self._starts[location.chr].size - 1, idx2 + 1)

                d1 = (
                    abs(mid_point - self._starts[location.chr][tidx1])
                    if tidx1 > -1
                    else sys.maxsize
                )

                d2 = (
                    abs(mid_point - self._starts[location.chr][tidx2])
                    if tidx2 <= max_index
                    else sys.maxsize
                )

                if d1 == sys.maxsize and d2 == sys.maxsize:
                    break

                if d1 <= d2:
                    ids = self._gene_start_map[location.chr][
                        self._starts[location.chr][tidx1]
                    ]
                    idx1 = tidx1
                else:
                    ids = self._gene_start_map[location.chr][
                        self._starts[location.chr][tidx2]
                    ]
                    idx2 = tidx2

                if are_unique_symbols:
                    unique_variant_ids = []

                    for id in ids:
                        gene_annotation = self._refseq_genes.get_gene(id)
                        symbol = gene_annotation.symbol

                        if symbol not in used:
                            unique_variant_ids.append(id)

                        used.add(symbol)
                else:
                    unique_variant_ids = ids

                # unique_variant_ids = list(
                #     filter(
                #         lambda variant_id: variant_id.split("#")[0] not in used,
                #         variant_ids,
                #     )
                # )

                found += len(unique_variant_ids)

                # print(used, variant_ids, len(variant_ids), n, tidx1, tidx2, mid_point)

                if len(unique_variant_ids) > 0:
                    variant_ids[min(d1, d2)].extend(unique_variant_ids)

                # used.update(
                #     map(lambda variant_id: variant_id.split("#")[0], variant_ids)
                # )
        ...
        # variant_ids = np.concatenate([before_variants, after_variants])

        # genes = [self._refseq.classify(v, mid_point) for v in variant_ids]
        ...
        for d in variant_ids:
            gene_dist_map[d].extend(
                [self._refseq.classify(v, mid_point) for v in variant_ids[d]]
            )
        ...
        # print("genes", [str(x) for x in genes_by_dist], len(genes_by_dist))

        return np.array(genes_by_dist)
...
class RefSeqStart(RefSeqTss):
    ...
    def __init__(
        ...
        super().__init__(
            "refseq-tss-start", file, refseq_genes, prom_ext_5p, prom_ext_3p
        )
        ...
        for chr in self._starts:
            for i in range(0, len(self._starts[chr])):
                ...
                for variant_id in self._gene_start_map[chr][start]:
                    self._gene_to_start_index_map[chr][variant_id] = i
        ...
        self._header.append(f"Relative To Closest Gene ({self._promoter_type})")
...
class RefSeqEnd(RefSeqTss):
    ...
    def __init__(
        ...
        print(
            f"RefSeqEnd: Loading from {file} {prom_ext_5p} {prom_ext_3p}...",
            file=sys.stderr,
        )
...
class OverlapTss(genomic.Annotation):
    ...
    def __init__(self, file: str, block_size: int = 100):
        ...
        for line in f:
            ...
            gene = genes.RefSeqGene(refseq, entrez, symbol, strand, chr, s, s)
